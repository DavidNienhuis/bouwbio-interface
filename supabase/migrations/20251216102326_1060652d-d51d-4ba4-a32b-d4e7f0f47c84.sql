-- Add source_files column to knowledge_bank table
ALTER TABLE public.knowledge_bank 
ADD COLUMN IF NOT EXISTS source_files JSONB DEFAULT NULL;

-- Drop the old function first
DROP FUNCTION IF EXISTS public.update_knowledge_bank(text, text, text, jsonb, jsonb);

-- Create updated function with source_files parameter
CREATE OR REPLACE FUNCTION public.update_knowledge_bank(
  ...
  p_result jsonb,
  p_source_files jsonb DEFAULT NULL
)
RETURNS void
LANGUAGE plpgsql
SECURITY DEFINER
...
BEGIN
  ...
  INSERT INTO public.knowledge_bank (
    ...
    latest_result, validation_count, source_files
  )
  VALUES (
    p_ean_code,
    p_product_name,
  ...
  SET
    ...
    -- IMPORTANT: Keep existing source_files, don't overwrite with new ones
    source_files = COALESCE(knowledge_bank.source_files, EXCLUDED.source_files);
END;
$function$;